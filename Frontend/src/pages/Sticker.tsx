  ...
  return (
    <div className="flex flex-col h-full min-h-0 gap-4">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 min-h-0">
        ...
        <Card variant="elevated" className="lg:col-span-3 flex flex-col overflow-hidden h-full shadow-sm">
          ...
          <div className="p-3 border-b border-neutral-100">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-9 h-9 text-sm bg-neutral-50/50 focus:bg-white transition-colors"
                autoFocus
              />
            </div>
          </div>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      selectedSku === p.sku 
                        ...
                        : 'hover:bg-neutral-50 active:bg-neutral-100 border-l-2 border-l-transparent'
                    )}
                  >
                    <div className={cn(
                      "w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors", 
                      selectedSku === p.sku ? 'bg-brand-100 text-brand-600' : 'bg-neutral-100 text-neutral-400'
                    )}>
                    ...
                    <div className="flex-1 min-w-0 pt-0.5">
                      <div className="flex items-center justify-between mb-0.5">
                        <p className={cn(
                          ...
                          selectedSku === p.sku ? 'text-brand-900 font-bold' : 'text-neutral-900 font-semibold'
                        )}>
                          {p.sku}
                        </p>
        ...
        <Card variant="elevated" className="lg:col-span-6 flex flex-col overflow-hidden h-full shadow-md z-10 border-neutral-200 ring-1 ring-black/[0.02]">
          ...
          <CardContent className="flex-1 p-0 flex flex-col relative bg-neutral-50">
            ...
            <div className="relative flex-1 flex flex-col items-center justify-center p-8 overflow-auto custom-scrollbar">
              {!product ? (
                <div className="flex flex-col items-center justify-center text-center max-w-sm">
                  <div className="w-20 h-20 bg-white shadow-sm ring-1 ring-neutral-200 rounded-full flex items-center justify-center mb-5 relative group">
                    <Tag className="w-8 h-8 text-neutral-300 group-hover:text-brand-400 transition-colors duration-300 absolute" />
                    <div className="absolute inset-0 border-2 border-neutral-200 border-dashed rounded-full group-hover:border-brand-300 animate-[spin_10s_linear_infinite]" />
                  </div>
                  <h4 className="text-lg font-heading font-bold text-neutral-800 mb-2">Canvas Ready</h4>
                ...
                <div className="flex flex-col items-center animate-in fade-in zoom-in-95 duration-300">
                  ...
                  <div 
                    className={cn(
                      "bg-white rounded-[4px] shadow-2xl overflow-hidden ring-1 ring-neutral-200/80 transition-all duration-500 relative select-none group", 
                    ...
                    <div className="p-3 h-full flex flex-col relative z-10">
                      <div className="flex justify-between items-start mb-2 pb-2 border-b border-neutral-100">
                        ...
                        {qrDataUrl && stickerSize !== '50x50' && (
                          <div className="bg-white p-0.5 rounded-sm ring-1 ring-neutral-100 shadow-sm shadow-neutral-200/20">
                      ...
                      <div className="flex-1 mt-1">
                        <div className="mb-1.5 focus-within:ring-2 focus-within:ring-brand-100 rounded-sm">
                          ...
                          <div className="font-mono text-sm font-bold text-neutral-900 leading-tight">{product.sku}</div>
                        </div>
                        {stickerSize !== '50x50' && (
                          <div className="grid grid-cols-2 gap-2 mt-2">
                            <div>
                              <div className="text-[7px] text-neutral-400 uppercase font-semibold tracking-wider">MRP</div>
                              <div className="font-bold text-xs text-neutral-800">₹{Math.max(product.mrpManual, product.mrpTally)}</div>
                            </div>
                            <div>
                              <div className="text-[7px] text-neutral-400 uppercase font-semibold tracking-wider">Expiry (Best Before)</div>
                              <div className="text-xs text-neutral-800 font-medium">{product.bestBefore}</div>
                            </div>
                          </div>
                        )}
                      </div>
                      <div className="mt-auto w-full flex justify-center bg-white px-2 py-1 -mx-2 -mb-1">
                  ...
                  <div className="mt-8 bg-white shadow-lg shadow-neutral-200/30 rounded-full border border-neutral-100 p-1.5 flex items-center gap-1">
                    <Button 
                      onClick={addToQueue} 
                      className="rounded-full px-5 hover:scale-105 active:scale-95 transition-all text-sm font-semibold shadow-sm"
                    ...
                    <Button 
                      ...
                      className="rounded-full hover:bg-neutral-100 text-neutral-600 px-4 text-sm"
            ...
            {product && (
              <div className="border-t border-neutral-200/60 bg-white px-4 py-3 flex items-center justify-between text-xs animate-in slide-in-from-bottom-2 duration-300 relative z-20">
                <div className="flex items-center gap-4 text-neutral-600">
                  ...
                  <span className="font-medium">Net Qty: <span className="text-neutral-900">{product.netQty} N</span></span>
                  <div className="w-px h-4 bg-neutral-200 hidden sm:block"></div>
                  <span className="font-medium hidden sm:block">Location: <span className="text-neutral-900 border border-neutral-200 rounded px-1.5 font-mono bg-neutral-50">{product.rack}-{product.shelf}-{product.bin}</span></span>
                </div>
              </div>
            )}
          </CardContent>
        ...
        <div className="lg:col-span-3 flex flex-col gap-4 min-h-0">
          <Card variant="elevated" className="flex-1 flex flex-col overflow-hidden shadow-sm">
            <CardHeader className="py-3 px-4 border-b border-neutral-100 bg-neutral-50/50">
              <div className="flex items-center justify-between">
                ...
                {printQueue.length > 0 && (
                  <Badge variant="primary" className="bg-brand-100 text-brand-700 animate-in zoom-in-50">{totalStickers} Labels</Badge>
                )}
              </div>
            </CardHeader>
            <CardContent className="flex-1 overflow-y-auto p-0 scrollbar-thin bg-neutral-50/20">
              {printQueue.length === 0 ? (
                ...
                <div className="divide-y divide-neutral-100/60 p-2 space-y-2">
                  {printQueue.map(item => (
                    <div key={item.id} className="bg-white p-3 rounded-lg border border-neutral-100 shadow-sm flex flex-col gap-2 group animate-in slide-in-from-right-2">
                      <div className="flex items-start justify-between">
                        <div className="min-w-0 pr-2">
                          <p className="font-mono text-sm font-bold text-neutral-900 truncate">{item.sku}</p>
                          <p className="text-[10px] text-neutral-500 uppercase font-semibold">Qty: {item.quantity}</p>
                        </div>
                        <button onClick={() => removeFromQueue(item.id)} className="w-6 h-6 rounded-md text-neutral-400 opacity-0 group-hover:opacity-100 hover:bg-danger-50 hover:text-danger-600 flex items-center justify-center transition-all bg-white shadow-sm ring-1 ring-neutral-100">
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      </div>
                      
                      <div className="flex items-center justify-between bg-neutral-50 rounded-md p-1 border border-neutral-100">
                        <button onClick={() => updateQueueQuantity(item.id, -1)} className="w-7 h-7 rounded bg-white shadow-sm text-neutral-600 hover:text-brand-600 hover:border-brand-200 border border-neutral-200 flex items-center justify-center transition-colors active:bg-neutral-100">
                          <Minus className="w-3.5 h-3.5" />
                        </button>
                        <span className="w-10 text-center text-sm font-bold text-neutral-800">{item.quantity}</span>
                        <button onClick={() => updateQueueQuantity(item.id, 1)} className="w-7 h-7 rounded bg-white shadow-sm text-neutral-600 hover:text-brand-600 hover:border-brand-200 border border-neutral-200 flex items-center justify-center transition-colors active:bg-neutral-100">
            ...
            {printQueue.length > 0 && (
              <CardFooter className="p-3 border-t border-neutral-100 bg-white mt-0">
                <div className="flex flex-col w-full gap-2">
                  <div className="flex items-center justify-between px-1 mb-1">
                    <span className="text-xs text-neutral-500">Total Labels:</span>
                    <span className="text-sm font-bold text-neutral-900">{totalStickers}</span>
                  </div>
                  <div className="flex gap-2 w-full">
                    <Button variant="outline" onClick={clearQueue} className="flex-1 shadow-sm text-xs bg-white" size="sm">
                      Clear
                    </Button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-neutral-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-0 overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-neutral-100 flex items-center justify-between bg-neutral-50/50">
              ...
              <button onClick={() => setShowPreview(false)} className="w-8 h-8 rounded-full hover:bg-neutral-200/60 flex items-center justify-center text-neutral-500 transition-colors">
                <span className="text-xl leading-none">&times;</span>
              </button>
            </div>
            
            <div className="bg-neutral-100/50 p-10 flex justify-center relative overflow-hidden">
               {/* Pattern */}
               <div className="absolute inset-0 bg-[radial-gradient(#cbd5e1_1px,transparent_1px)] [background-size:16px_16px] opacity-30"></div>
               
              <div className="bg-white rounded-xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.15)] p-4 w-[280px] ring-1 ring-neutral-200/50 relative z-10 transform scale-110">
                ...
                <div className="space-y-2 mt-4">
                  <div className="flex flex-col border-t border-neutral-100 pt-2">
                    ...
                    <div className="font-mono text-sm font-bold text-neutral-800 tracking-tight">{products.find(p => p.sku === selectedSku)?.sku}</div>
                  </div>
            ...
            <div className="p-4 bg-white flex gap-3 border-t border-neutral-100">
