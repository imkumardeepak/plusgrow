...
export const WarehouseMap = memo(function WarehouseMap() {
   ...
   return (
      <div className="flex flex-col h-full min-h-0 gap-3">
         ...
         <Card variant="glass" className="py-2 px-3 shrink-0">
            <div className="flex items-center justify-between">
               <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg bg-orange-500/10 border border-orange-500/20 flex items-center justify-center shadow-[0_0_10px_rgba(249,115,22,0.1)]">
                     <Map className="w-4 h-4 text-orange-400" />
                  </div>
                  <div>
                     <h1 className="text-sm font-bold text-white leading-none tracking-tight">Facility Mapping</h1>
                     <p className="text-[10px] text-neutral-500 font-medium mt-0.5">Asset & Inventory Locator</p>
                  </div>
               </div>
               <div className="flex items-center gap-2">
         ...
         <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3 min-h-0">
            ...
            {/* Left Column: Located SKU List - Compacted */}
            <Card variant="elevated" className="lg:col-span-3 flex flex-col h-full overflow-hidden border-white/5 bg-white/[0.02]">
               <CardHeader className="py-2 px-3 border-b border-white/10 bg-orange-500/5 shrink-0">
                  <div className="flex items-center justify-between">
                     <CardTitle size="sm" className="flex items-center gap-1.5">
                        <Crosshair className="w-3.5 h-3.5 text-orange-400" />
                        <span className="text-orange-300 font-bold text-[11px] uppercase tracking-wider">Coordinates</span>
                     </CardTitle>
                     <Badge variant="primary" size="sm" className="bg-orange-500/10 text-orange-400 border-orange-500/20 text-[9px]">
                        {filteredStock.length}
                     </Badge>
                  </div>
               </CardHeader>
               <CardContent className="flex-1 overflow-y-auto p-1.5 scrollbar-thin relative z-10">
                  <div className="space-y-1">
                        ...
                        >
                           <div className="flex items-start justify-between">
                              <div className="flex items-center gap-2">
                                 ...
                                 <div className="min-w-0">
                                    <h4 className="text-[11px] font-bold text-white truncate">{item.sku}</h4>
                                    <p className="text-[9px] uppercase font-bold text-neutral-500 truncate">{item.title}</p>
                                 </div>
                              </div>

                              <Badge variant={selectedSku === item.sku ? "warning" : "default"} size="sm" className={cn(
                                 "text-[9px] h-4 tracking-tighter px-1 shadow-none",
                                 selectedSku !== item.sku && "bg-white/5 text-neutral-500 border-white/10"
                              )}>
                                 {item.quantity}
                              </Badge>
                           </div>
                           ...
                           <div className="flex items-center justify-between mt-0.5 px-0.5">
                              ...
                              <ArrowRight className={cn(
                                 ...
                                 selectedSku === item.sku ? "text-orange-500 translate-x-0.5" : "text-neutral-500 opacity-0"
                              )} />
                           </div>
                        </div>
                     ))}
                     ...
                     {filteredStock.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-10 text-center opacity-40">
                           ...
                           <h3 className="text-[10px] font-bold text-neutral-300">No Asset Found</h3>
                        </div>
                     )}
                  </div>
               </CardContent>
            ...
            {/* Right Column: Visual Map - Compacted */}
            <Card variant="elevated" className="lg:col-span-9 flex flex-col h-full overflow-hidden border-white/5 bg-neutral-950">
               <CardHeader className="py-2 px-3 border-b border-white/5 bg-white/[0.02] shrink-0">
                  <div className="flex items-center justify-between">
                     <CardTitle size="sm" className="flex items-center gap-1.5 text-white">
                        <Map className="w-3.5 h-3.5 text-orange-400" />
                        <span className="text-[11px] uppercase tracking-widest font-bold">Interactive Floor Plan</span>
                     </CardTitle>
                     <div className="flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.8)] animate-pulse"></span>
                        <span className="text-[9px] font-bold text-neutral-500 uppercase tracking-widest">System Live</span>
                     </div>
                  </div>
               </CardHeader>

               <CardContent className="flex-1 p-4 relative flex items-center justify-center bg-[radial-gradient(#222_1px,transparent_1px)] [background-size:15px_15px] overflow-auto">
                  ...
                  <div className="absolute left-4 top-4 bottom-4 w-10 border border-neutral-800 border-dashed rounded-lg bg-neutral-900/30 flex flex-col items-center justify-around py-6 opacity-40">
                     <div className="-rotate-90 text-[8px] font-bold tracking-[0.2em] text-neutral-500 uppercase whitespace-nowrap">Receiving</div>
                     <div className="w-4 h-px bg-neutral-800"></div>
                     <div className="-rotate-90 text-[8px] font-bold tracking-[0.2em] text-neutral-500 uppercase whitespace-nowrap">Dispatch</div>
                  </div>
                        ...
                        return (
                           ...
                           >
                              ...
                              <div className={cn(
                                 ...
                                 isHighlighted ? "bg-orange-500 text-white border-orange-600" : "bg-neutral-900 text-neutral-500 border-neutral-800"
                              )}>
                                 <span className="font-heading font-bold tracking-widest text-[11px]">{rack.name}</span>
                              </div>
                                       ...
                                       <div
                                          ...
                                          className={cn(
                                             ...
                                             isShelfHighlighted
                                                ...
                                                : (isHighlighted ? "border-orange-500/10 bg-neutral-950/50" : "border-neutral-800/50 bg-neutral-950/50")
                                          )}
                                       >
                                          {isShelfHighlighted && (
                                             <Badge variant="warning" className="text-[7px] bg-orange-500 text-white px-1 py-0 shadow-lg scale-90">Target</Badge>
                                          )}
                                       </div>
                                    );
                                 })}
                              ...
                              {totalItems > 0 && (
                                 <div className={cn(
                                    ...
                                    isHighlighted ? "bg-orange-600 text-white" : "bg-neutral-800 text-neutral-500"
                                 )}>
                                    {totalItems} Units
                                 </div>
                              )}
                  ...
                  {selectedSku && (
                     <div className="absolute bottom-4 right-4 max-w-[200px] bg-neutral-900/90 backdrop-blur-md border border-white/10 p-2.5 rounded-lg shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
                        <div className="flex items-center gap-2 border-b border-white/5 pb-2 mb-2">
                           ...
                           <div className="min-w-0">
                              <p className="text-[8px] font-bold text-neutral-500 uppercase tracking-widest">Asset Tracker</p>
                              <p className="text-[10px] font-bold text-white leading-tight truncate">{selectedSku}</p>
                           </div>
                        </div>
                        <div className="space-y-1">
                           {locatedStock.filter(s => s.sku === selectedSku).slice(0, 3).map((s, i) => (
                              <div key={i} className="flex justify-between items-center text-[9px] font-mono bg-white/[0.02] px-1.5 py-1 rounded border border-white/5">
                                 <span className="text-neutral-500">{s.rack}-{s.shelf}-{s.bin}</span>
                                 <span className="text-orange-400 font-bold">{s.quantity}</span>
                              </div>
                           ))}
                           {locatedStock.filter(s => s.sku === selectedSku).length > 3 && (
                              <p className="text-[8px] text-center text-neutral-600 mt-1">+{locatedStock.filter(s => s.sku === selectedSku).length - 3} more</p>
                           )}
                        </div>
                     </div>
                  )}
